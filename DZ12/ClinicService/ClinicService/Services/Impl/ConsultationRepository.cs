    ...
    public class ConsultationRepository : IConsultationRepository
    {
        ...
        public int Create(Consultation item)
        {
            using (SqliteConnection connection = new SqliteConnection(connectionString))
            {
                ...
                command.CommandText = "INSERT INTO Consultations(ClientId, PetId, ConsultationsDate, Description) VALUES(@ClientId, @SPetId, @ConsultationsDate, @Description)";
                command.Parameters.AddWithValue("@ClientId", item.ClientId);
                command.Parameters.AddWithValue("@PetId", item.PetId);
                command.Parameters.AddWithValue("@ConsultationsDate", item.ConsultationsDate.Ticks);
        ...
        public int Update(Consultation item)
        {
            using (SqliteConnection connection = new SqliteConnection(connectionString))
            {
                ...
                SqliteCommand command = connection.CreateCommand();
                command.CommandText = "UODATE Consultations SET ClientId = @ClientId, PetId = @PetId, ConsultationsDate = @ConsultationsDate, Description = @Description WHERE ConsultationId = @ConsultationId)";
            ...
            using (SqliteConnection connection = new SqliteConnection(connectionString))
            {
                ...
                while (reader.Read())
                {
                    Consultation consultation = new Consultation
                    {
                        ...
                        ConsultationsDate = new DateTime(reader.GetInt64(3)),
